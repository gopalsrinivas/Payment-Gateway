...
const ProductCard = ({ product }) => {
  ...
  return (
    <article className="flex h-full flex-col rounded-md border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md">
      <div className="flex aspect-[4/3] items-center justify-center rounded-md bg-gradient-to-br from-emerald-50 to-slate-100 text-sm font-medium text-slate-500">
      ...
      <div className="mt-4 flex flex-1 flex-col">
        ...
        <div className="mt-auto grid gap-2 pt-4 sm:grid-cols-2">
          ...
          {canAdd ? (
            <Button type="button" disabled={mutating} onClick={handleAdd} aria-label={`Add ${product.name} to cart`} className="flex-1">
