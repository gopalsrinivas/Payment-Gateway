...
const CustomerNavbar = () => {
  ...
  const navLinks = (
    <>
      <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/" onClick={close}>
        Home
      </Link>
      <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/products" onClick={close}>
        Browse Products
      </Link>
      {isCustomer && (
        <>
          <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/dashboard" onClick={close}>
            Customer Dashboard
          </Link>
          <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/orders" onClick={close}>
            My Orders
          </Link>
          <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/payments" onClick={close}>
            My Payments
          </Link>
          <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/profile" onClick={close}>
            My Profile
          </Link>
          <Link className="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-brand" href="/cart" aria-label={`My Cart with ${itemCount} items`} onClick={close}>
            <FiShoppingCart className="inline" /> <span className="ml-1">My Cart</span>
            <span className="ml-1 rounded-full bg-brand px-2 py-0.5 text-xs text-white">{itemCount}</span>
          </Link>
        </>
      )}
      {isAdmin && (
